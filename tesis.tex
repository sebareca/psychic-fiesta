...
\begin{obs}
	...
	\[
		\eta_{A \ox B}(1) = \eta_A(1) \ox \eta_B(1).
	\]
\end{obs}
...
\begin{definition}
	Sea $C$ una coálgebra. La \emph{coálgebra coopuesta} $C^{cop}$ tiene a $C$ como espacio vectorial subyacente, 				comultiplicación $\Delta^{cop} = \tau_{C \ox C} \circ \Delta$ y $\veps^{cop} = \veps$.
\end{definition}
